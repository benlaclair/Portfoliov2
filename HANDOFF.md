...
- **GSAP + CSS transform conflict (non-obvious).** If an element has a CSS `transform: translateY(100%)` and you animate it with `yPercent`, GSAP parses the CSS into a pixel `y` value that stacks with the `yPercent` offset, sending the element off-screen. Fix: call `gsap.set(el, { y: 0, yPercent: 100 })` before any tween so GSAP owns the transform with `y: 0`. The hero curtain (`#hero-curtain`) has this initialisation at the top of the `if (!reduceMotion.current)` block — don't remove it. See ANIMATIONS.md "Hero scroll gate" for full details.
